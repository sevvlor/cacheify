...
#### Only Cache GET And HEAD (`onlyCacheGetAndHead`)
*Default: true*

If true (the default), any request whose method is not `GET` or `HEAD` unconditionally bypasses the cache (both reading and writing) - matching Varnish's default `vcl_recv` behaviour of only ever dealing with GET/HEAD. The underlying `cachecontrol` library already hard-blocks `PUT`/`DELETE`/`CONNECT`/`OPTIONS`/`TRACE` regardless of this setting, but it permits `POST` when the response carries explicit freshness information, per RFC 7234 (intended for niche cases like cacheable search results). Because this plugin's cache key never incorporates the request body, enabling POST caching is dangerous for any POST-based API that distinguishes requests by body rather than URL - most notably GraphQL, which typically serves every query and mutation through a single POST endpoint. If such an origin ever sets explicit `Cache-Control` on that endpoint, every distinct request would collapse onto the same cache key, and the first response would be served to every other request regardless of what its body actually asked for. Disable this only if you are certain no POST route behind this middleware distinguishes responses by request body.
